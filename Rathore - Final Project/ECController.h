...
public:
    ...
    void ToggleFillMode();
    //ECController(ECGraphicViewImp& viewIn) : view(viewIn), listShapes(), EditMode(0), startX(0), startY(0), endX(0), endY(0), cursorX(0), cursorY(0), ToDraw(false), ellipse(false), fillMode(false) {}

    bool GetFillMode() const;
    void OnKeyPress(char key);
    void SaveStateForUndo();

    std::vector <ECShape* > getShapes();
    bool loadShapes(const std::string& filename);
...
private:
    ...
    std::stack<std::vector<ECShape*>> undoStack;
    std::stack<std::vector<ECShape*>> redoStack;
    bool fillMode;
